...
import { Button, Card, Form, Image, Input, Space, Tag, Typography } from "antd";
import dayjs from "dayjs";
import { useEffect, useMemo, useState } from "react";

import { fetchMyDeductionLogs, type CreditLog } from "@/services/api/deduction-logs";
...
export default function MyDeductionLogsPage() {
    ...
    const query = useQuery({
        queryKey: ["user", "deduction-logs", token, keyword, page, pageSize],
        queryFn: () => fetchMyDeductionLogs(token, { keyword, page, pageSize }),
    ...
    useEffect(() => {
        ...
        const message = query.error instanceof Error ? query.error.message : "读取我的流水失败";
                    ...
                    <Form layout="vertical">
                        <Form.Item label="关键词">
                            <Space.Compact style={{ width: 520, maxWidth: "100%" }}>
                                <Input value={keywordText} placeholder="搜索任务、提示词、图片链接" allowClear onChange={(event) => setKeywordText(event.target.value)} onPressEnter={() => search()} />
