...
export const IMAGE_PRESET_EDIT_CONFIG = {
    ...
    logoVectorize: {
        ...
        error: "Logo 矢量失败",
        prompt: "请基于参考图片，重新绘制为适合后续转 SVG / AI / EPS / CDR 初稿的高清纯色 Logo 矢量友好稿。\n\n核心目标：最大程度保持原图的视觉识别效果、文字内容、字形轮廓、标识结构和版式关系，只做高清放大、边缘清理、色彩整理和形状闭合，不要重新设计。\n\n适用对象包括品牌 Logo、文字标识、门头字牌、店招灯箱、产品铭牌、包装标识、导视标识、图形商标和简单图文组合。\n\n要求：\n1. 严格保留原图的整体比例、构图、字距、行距、倾斜角度、粗细关系、圆角、尖角、内孔、镂空、负形、波浪线、边框和图形位置。\n2. 保持所有可识别文字的原始内容和排版关系，不新增文字，不改字，不替换品牌元素，不改变 Logo 主体识别特征。\n3. 按原图保留必要的功能性主色层：主图形/主文字色、辅助图形色、必要阴影或层次色、背景白色；不要机械减色导致识别特征丢失。\n4. 允许保留有设计意义的大面积层次色块，不要保留抗锯齿过渡色、半透明边缘色和无意义渐变色阶。\n5. 主体边缘必须干净连续，由清晰路径边界决定；不要在边缘生成任何浅色碎块、细小补丁、杂色描边、噪点块或伪高光。\n6. 对轻微模糊、压缩噪点、半透明毛边和阴影进行清理；相近的边缘杂色必须合并到最近的主色，但有设计意义的内部层次色块要保留。\n7. 字母和图形中的白色内孔、镂空区域必须保持为真实负形关系，不要用白色或浅色覆盖色块伪装；避免产生多余碎块和毛边。\n8. 画面居中，四周留白均匀，主体完整，不裁切，不拉伸，不透视变形。\n\n避免：照片质感、真实反光、复杂渐变、投影、纹理噪点、脏背景、模糊边缘、锯齿、手绘抖动、卡通化、贴纸化、重新设计、改变比例、修改文字、浅色边缘碎块、添加水印。",
    },
    decompose: {
