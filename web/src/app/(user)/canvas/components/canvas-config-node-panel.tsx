...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const credits = requestCreditCost({ channelMode: config.channelMode, modelCosts, model: config.model, count: mode === "image" ? count : 1, size: mode === "image" ? config.size : "", quality: mode === "image" ? config.quality : "" });
    ...
    const submit = () => {
        if (isRunning) return;
        if (!canGenerate) {
            message.warning(mode === "audio" ? "请先填写组装提示词，或连接一个文本节点" : "请先填写组装提示词，或连接提示词/参考素材");
            return;
        }
        onGenerate(node.id);
    };
