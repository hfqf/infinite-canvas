...
主应用镜像已内置 `png2svg-clean-node`、Node 依赖和 ImageMagick，用于画布图片节点“转矢量”的后端 SVG 转换。如果不使用 Docker 部署，需要在后端运行环境安装 Node.js、ImageMagick，并在 `png2svg-clean-node` 目录执行 `npm install`；可通过 `PNG2SVG_CLEAN_TOOL_DIR`、`PNG2SVG_CLEAN_NODE_PATH`、`PNG2SVG_CLEAN_PROFILE`、`PNG2SVG_CLEAN_TIMEOUT_SEC` 和 `IMAGE_MAGICK_PATH` 调整工具路径和参数。
