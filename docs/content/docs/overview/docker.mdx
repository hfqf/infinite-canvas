...
主应用镜像已内置 VTracer、ImageMagick 和 potrace，用于画布图片节点“转矢量”的后端 SVG 转换，以及“Logo矢量”的高清放大、限色清理、分色追踪和小组件边缘优化。如果不使用 Docker 部署，需要在后端运行环境安装 `vtracer`、ImageMagick、`potrace` 和 `mkbitmap`，或通过 `VTRACER_PATH`、`IMAGE_MAGICK_PATH` 指定可执行文件路径；Logo 最大量化色数可通过 `LOGO_VECTORIZE_COLORS` 调整。
