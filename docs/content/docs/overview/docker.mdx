...
主应用镜像已内置 VTracer 和 ImageMagick，用于画布图片节点“转矢量”的后端 SVG 转换，以及“Logo矢量”的高清放大和限色清理。如果不使用 Docker 部署，需要在后端运行环境安装 `vtracer` 和 ImageMagick，或通过 `VTRACER_PATH`、`IMAGE_MAGICK_PATH` 指定可执行文件路径。
