...
| `gift_credits` | number | 系统赠送额度余额，仅可用于支持赠送额度的模型 |
...
| `defaultModel` | string | 默认模型，从启用且可路由的公开模型中选择 |
| `defaultImageModel` | string | 默认图片模型，从启用且 `capability=image` 的公开模型中选择 |
| `defaultVideoModel` | string | 默认视频模型，从启用且 `capability=video` 的公开模型中选择 |
| `defaultTextModel` | string | 默认文本模型，从启用且 `capability=text` 的公开模型中选择 |
| `systemPrompt` | string | 系统提示词 |
| `allowCustomChannel` | bool | 当前默认关闭，前端统一走后端中转接口 |
...
| `enabled` | bool | 是否开放给前端使用 |
| `giftEligible` | bool | 是否允许使用系统赠送额度 |
...
| `credits` | number | 命中该服务商路由时实际扣除的算力点 |
| `weight` | number | 同一公开模型命中多个可用路由时的加权随机权重 |
| `enabled` | bool | 是否启用该路由 |
...
后端请求模型时，先按公开模型名筛选启用的公开规格，再从可用渠道路由中按 `weight` 加权随机选择一个路由，并在转发上游前把请求体里的 `model` 改写为该路由的 `upstreamModel`。

### credit_logs

用户算力点变更流水表。当前记录后台手动调整、模型调用预扣和模型调用失败返还。模型调用涉及赠送额度时，`amount` 记录本次总额度变动，`balance` 记录变动后的通用算力点余额，赠送额度扣减和返还明细写入 `extra.giftCredits`。
...
| `remark` | string | 备注 |
| `extra` | json | 扩展信息；模型扣费会记录公开模型、上游模型、渠道、通用额度和赠送额度明细 |
| `created_at` | string | 创建时间 |
