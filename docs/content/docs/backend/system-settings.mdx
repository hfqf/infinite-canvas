...
```json
{
  "modelChannel": {
    ...
    "modelCosts": [
      ...
      { "model": "gpt-image-2", "credits": 10, "supports4K": true }
    ],
  ...
  "image": {
    "referenceCompressionQuality": 0.8
  },
  "canvas": {
    "toolCosts": [
...
| --- | --- | --- |
| `model` | string | 模型名称 |
| `credits` | number | 模型默认积分单价；图片请求中表示非 4K 单张基础积分 |
| `supports4K` | boolean | 图片模型是否支持 4K，默认 `true`；支持时用户选择 4K 清晰度的单张基础积分为 `credits + 3`，不支持时前端隐藏画布 4K 清晰度按钮 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `toolCosts` | object[] | 画布工具栏功能积分配置，未配置的工具默认 `0` 积分；点击工具时按该配置扣费 |

`toolCosts` 每项字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `tool` | string | 工具 ID，例如 `crop`、`split`、`upscale`、`superResolve` |
| `credits` | number | 使用该工具消耗的积分，最小为 `0`；设为 `0` 时不扣费 |
...
后端调用模型时，会从已启用、已配置 `baseUrl` 和 `apiKey`、且 `models` 包含目标模型的渠道中选择一个。图片生成和图片编辑请求遇到网络错误、`429` 或 `5xx` 时，会在同一次请求内尝试同模型的其他可用渠道；达到 `failureThreshold` 的渠道会按本次图片请求超时时长临时跳过，超时时长为 `参考图数量 * 60 + 基础秒数`，基础秒数非 4K 为 `120`、4K 为 `180`。4K 判断按业务清晰度选择，不按裸 API 尺寸推断。`400`、`401`、`403` 等参数或权限错误不会自动切换备用渠道。
